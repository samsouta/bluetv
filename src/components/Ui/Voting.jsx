    ...
    return (
        <div className=" flex gap-x-4" >
            <Button onClick={() => handleVote('like', "1")} isIconOnly color='primary' className={`  bg-white/5 montserrat`}>
                <FaRegThumbsUp className={` ${isLike ? " hidden" : " visible"} text-2xl text-[#c3c6c3]`} />
                <FaThumbsUp className={` ${isLike ? " visible" : " hidden"} text-2xl text-[#c3c6c3]`} />
            </Button>
            <Button onClick={() => handleVote('dislike', "2")} isIconOnly color='primary' className=" bg-white/5 montserrat ">
                <FaRegThumbsDown className={` ${isDislike ? " hidden" : " visible"} text-2xl text-[#c3c6c3]`} />
                <FaThumbsDown className={` ${isDislike ? " visible" : " hidden"} text-2xl text-[#c3c6c3]`} />
            </Button>
        </div>
    );
}
